...
Page {
    ...
    function loadAlbum() {
        if (!componentReady || albumId.length === 0)
            return

        busy = true
        errorText = ""
        tracksModel.clear()
        backend.album(String(albumId))
    }
    ...
    function refreshFavourite() {
        ...
        favouriteBusy = true
        backend.itemFavouriteState("album", albumId)
    }

    function toggleFavourite() {
        if (albumId.length === 0 || favouriteBusy)
            return

        favouriteBusy = true
        backend.setItemFavourite(
            "album",
            albumId,
            !favourite
        )
    ...
    SilicaListView {
        ...
        header: Column {
            ...
            Item {
                ...
                Item {
                    ...
                    Label {
                        anchors.centerIn: parent
                        text: page.favouriteBusy
                              ? "\u00B7"
                              : page.favourite ? "\u2665" : "\u2661"
                        color: page.favourite
                    ...
                    MouseArea {
                        id: albumHeartMouse
                        anchors.fill: parent
                        enabled: !page.favouriteBusy
                        onClicked: page.toggleFavourite()
                    }

                    scale: albumHeartMouse.pressed ? 0.88 : 1.0
        ...
        footer: Column {
            ...
            BackgroundItem {
                ...
                Label {
                    anchors.centerIn: parent
                    text: "Discover " + page.albumArtist
                    color: FiatPonsTheme.accent
                    font.pixelSize: Theme.fontSizeSmall
                }
            }
