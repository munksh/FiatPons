mod facade;
use std::ffi::{CStr, CString};
use std::os::raw::c_char;
use std::sync::Mutex;
use once_cell::sync::Lazy;
use tokio::runtime::Runtime;
use facade::{
    Core,
    FpTrack,
    FpAlbum,
    FpArtist,
    FpPlaylist,
    FavouritesPayload,
};

static RT:   Lazy<Runtime>             = Lazy::new(|| Runtime::new().expect("tokio runtime"));
static CORE: Lazy<Mutex<Option<Core>>> = Lazy::new(|| Mutex::new(None));

fn token_path() -> Option<String> {
    let home = std::env::var("HOME").ok()?;
    Some(format!("{home}/.local/share/se.munkstolen/harbour-fiatpons/token"))
}

fn ensure_core(slot: &mut Option<Core>) -> Result<(), String> {
    if slot.is_some() { return Ok(()); }
    let path = token_path().ok_or("no HOME set")?;
    let token = std::fs::read_to_string(&path)
        .map_err(|e| format!("no token file at {path}: {e}"))?
        .trim().to_string();
    if token.is_empty() { return Err("token file is empty".into()); }
    let core = RT.block_on(async {
        let c = Core::new().await?;
        c.login_with_token(&token).await?;
        Ok::<Core, String>(c)
    })?;
    *slot = Some(core);
    Ok(())
}

// ---- FFI surface ----

#[no_mangle]
pub extern "C" fn fp_search(query: *const c_char) -> *mut c_char {
    to_c(build_search(query))
}

#[no_mangle]
pub extern "C" fn fp_search_albums(query: *const c_char) -> *mut c_char {
    to_c(build_search_albums(query))
}

#[no_mangle]
pub extern "C" fn fp_search_artists(query: *const c_char) -> *mut c_char {
    to_c(build_search_artists(query))
}

#[no_mangle]
pub extern "C" fn fp_stream_url(track_id: u64) -> *mut c_char {
    to_c(build_stream(track_id))
}

#[no_mangle]
pub extern "C" fn fp_stream_url_quality(
    track_id: u64,
    quality: *const c_char,
) -> *mut c_char {
    to_c(build_stream_quality(track_id, quality))
}

#[no_mangle]
pub extern "C" fn fp_album(album_id: *const c_char) -> *mut c_char {
    to_c(build_album(album_id))
}

#[no_mangle]
pub extern "C" fn fp_artist(artist_id: u64) -> *mut c_char {
    to_c(build_artist(artist_id))
}

#[no_mangle]
pub extern "C" fn fp_radio_artist(artist_id: u64) -> *mut c_char {
    to_c(build_radio_artist(artist_id))
}

#[no_mangle]
pub extern "C" fn fp_user_playlists() -> *mut c_char {
    to_c(build_user_playlists())
}

#[no_mangle]
pub extern "C" fn fp_playlist(playlist_id: u64) -> *mut c_char {
    to_c(build_playlist(playlist_id))
}

#[no_mangle]
pub extern "C" fn fp_favourites(mode: *const c_char) -> *mut c_char {
    to_c(build_favourites(mode))
}

#[no_mangle]
pub extern "C" fn fp_oauth_url(
    port: u16,
) -> *mut c_char {
    to_c(build_oauth_url(port))
}

#[no_mangle]
pub extern "C" fn fp_login_code(
    code: *const c_char,
) -> *mut c_char {
    to_c(build_login_code(code))
}

#[no_mangle]
pub extern "C" fn fp_login_browser() -> *mut c_char {
    to_c(build_login_browser())
}

#[no_mangle]
pub extern "C" fn fp_logout() -> *mut c_char {
    to_c(build_logout())
}

#[no_mangle]
pub extern "C" fn fp_is_logged_in() -> *mut c_char {
    to_c(build_is_logged_in())
}

#[no_mangle]
pub extern "C" fn fp_discover_featured() -> *mut c_char {
    to_c(build_discover_featured())
}

#[no_mangle]
pub extern "C" fn fp_discover_albums(
    endpoint: *const c_char,
    genre_id: u64,
    offset: u32,
    limit: u32,
) -> *mut c_char {
    to_c(build_discover_albums(
        endpoint,
        genre_id,
        offset,
        limit,
    ))
}

#[no_mangle]
pub extern "C" fn fp_discover_genres() -> *mut c_char {
    to_c(build_discover_genres())
}

#[no_mangle]
pub extern "C" fn fp_create_playlist(
    name: *const c_char,
    initial_track_id: u64,
) -> *mut c_char {
    to_c(build_create_playlist(name, initial_track_id))
}

#[no_mangle]
pub extern "C" fn fp_add_track_to_playlist(
    playlist_id: u64,
    track_id: u64,
) -> *mut c_char {
    to_c(build_add_track_to_playlist(playlist_id, track_id))
}

#[no_mangle]
pub extern "C" fn fp_create_playlist_visible(
    name: *const c_char,
    initial_track_id: u64,
    is_public: bool,
) -> *mut c_char {
    to_c(build_create_playlist_visible(
        name,
        initial_track_id,
        is_public,
    ))
}

#[no_mangle]
pub extern "C" fn fp_remove_playlist_track(
    playlist_id: u64,
    playlist_track_id: u64,
) -> *mut c_char {
    to_c(build_remove_playlist_track(
        playlist_id,
        playlist_track_id,
    ))
}

#[no_mangle]
pub extern "C" fn fp_set_playlist_public(
    playlist_id: u64,
    is_public: bool,
) -> *mut c_char {
    to_c(build_set_playlist_public(
        playlist_id,
        is_public,
    ))
}

#[no_mangle]
pub extern "C" fn fp_save_playlist_order(
    playlist_id: u64,
    ordered_track_ids: *const c_char,
) -> *mut c_char {
    to_c(build_save_playlist_order(
        playlist_id,
        ordered_track_ids,
    ))
}

#[no_mangle]
pub extern "C" fn fp_item_favourite_state(
    item_type: *const c_char,
    item_id: *const c_char,
) -> *mut c_char {
    to_c(build_item_favourite_state(item_type, item_id))
}

#[no_mangle]
pub extern "C" fn fp_set_item_favourite(
    item_type: *const c_char,
    item_id: *const c_char,
    favourite: bool,
) -> *mut c_char {
    to_c(build_set_item_favourite(
        item_type,
        item_id,
        favourite,
    ))
}

#[no_mangle]
pub extern "C" fn fp_track_favourite_state(
    track_id: u64,
) -> *mut c_char {
    to_c(build_track_favourite_state(track_id))
}

#[no_mangle]
pub extern "C" fn fp_set_track_favourite(
    track_id: u64,
    favourite: bool,
) -> *mut c_char {
    to_c(build_set_track_favourite(track_id, favourite))
}

// ---- builders ----

fn read_cstr(p: *const c_char) -> Result<String, String> {
    if p.is_null() { return Err("null string".into()); }
    match unsafe { CStr::from_ptr(p) }.to_str() {
        Ok(s) => Ok(s.to_string()),
        Err(_) => Err("string not utf-8".into()),
    }
}

fn build_search(query: *const c_char) -> String {
    let q = match read_cstr(query) { Ok(s) => s, Err(e) => return err_json(&e) };
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().search(&q)) {
        Ok(tracks) => tracks_json(&tracks),
        Err(e) => err_json(&e),
    }
}

fn build_search_albums(query: *const c_char) -> String {
    let q = match read_cstr(query) { Ok(s) => s, Err(e) => return err_json(&e) };
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().search_albums(&q)) {
        Ok(albums) => albums_json(&albums),
        Err(e) => err_json(&e),
    }
}

fn build_search_artists(query: *const c_char) -> String {
    let q = match read_cstr(query) { Ok(s) => s, Err(e) => return err_json(&e) };
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().search_artists(&q)) {
        Ok(artists) => artists_json(&artists),
        Err(e) => err_json(&e),
    }
}

fn build_stream(track_id: u64) -> String {
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().stream_url(track_id)) {
        Ok(s) => serde_json::to_string(&s).unwrap_or_else(|e| err_json(&e.to_string())),
        Err(e) => err_json(&e),
    }
}

fn build_stream_quality(
    track_id: u64,
    quality: *const c_char,
) -> String {
    let quality = match read_cstr(quality) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .stream_url_quality(track_id, &quality)
    ) {
        Ok(stream) => serde_json::to_string(&stream)
            .unwrap_or_else(|error| {
                err_json(&error.to_string())
            }),
        Err(error) => err_json(&error),
    }
}

fn build_album(album_id: *const c_char) -> String {
    let id = match read_cstr(album_id) { Ok(s) => s, Err(e) => return err_json(&e) };
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().album(&id)) {
        Ok((album, tracks)) => album_json(&album, &tracks),
        Err(e) => err_json(&e),
    }
}

fn build_artist(artist_id: u64) -> String {
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().artist(artist_id)) {
        Ok((artist, albums)) => artist_json(&artist, &albums),
        Err(e) => err_json(&e),
    }
}

fn build_radio_artist(artist_id: u64) -> String {
    let mut slot = CORE.lock().unwrap();
    if let Err(e) = ensure_core(&mut slot) { return err_json(&e); }
    match RT.block_on(slot.as_ref().unwrap().radio_artist(artist_id)) {
        Ok(tracks) => tracks_json(&tracks),
        Err(e) => err_json(&e),
    }
}


fn build_user_playlists() -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(e) = ensure_core(&mut slot) {
        return err_json(&e);
    }

    match RT.block_on(slot.as_ref().unwrap().user_playlists()) {
        Ok(playlists) => playlists_json(&playlists),
        Err(e) => err_json(&e),
    }
}

fn build_playlist(playlist_id: u64) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(e) = ensure_core(&mut slot) {
        return err_json(&e);
    }

    match RT.block_on(slot.as_ref().unwrap().playlist(playlist_id)) {
        Ok((playlist, tracks)) => playlist_json(&playlist, &tracks),
        Err(e) => err_json(&e),
    }
}

fn build_favourites(mode: *const c_char) -> String {
    let mode = match read_cstr(mode) {
        Ok(s) => s,
        Err(e) => return err_json(&e),
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(e) = ensure_core(&mut slot) {
        return err_json(&e);
    }

    match RT.block_on(slot.as_ref().unwrap().favourites(&mode)) {
        Ok(payload) => favourites_json(&payload),
        Err(e) => err_json(&e),
    }
}


fn build_oauth_url(port: u16) -> String {
    let mut slot = CORE.lock().unwrap();

    if slot.is_none() {
        match RT.block_on(Core::new()) {
            Ok(core) => *slot = Some(core),
            Err(error) => {
                return format!("ERROR:{error}")
            }
        }
    }

    match RT.block_on(
        slot.as_ref().unwrap().oauth_url(port)
    ) {
        Ok(url) => url,
        Err(error) => format!("ERROR:{error}"),
    }
}

fn build_login_code(
    code: *const c_char,
) -> String {
    let code = match read_cstr(code) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut slot = CORE.lock().unwrap();

    if slot.is_none() {
        match RT.block_on(Core::new()) {
            Ok(core) => *slot = Some(core),
            Err(error) => return err_json(&error),
        }
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .login_with_code_and_save(&code)
    ) {
        Ok(display_name) => serde_json::json!({
            "ok": true,
            "display_name": display_name
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_login_browser() -> String {
    let mut slot = CORE.lock().unwrap();

    if slot.is_none() {
        match RT.block_on(Core::new()) {
            Ok(core) => *slot = Some(core),
            Err(error) => return err_json(&error),
        }
    }

    match RT.block_on(slot.as_ref().unwrap().login_browser_and_save()) {
        Ok(display_name) => serde_json::json!({
            "ok": true,
            "display_name": display_name
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_logout() -> String {
    let mut slot = CORE.lock().unwrap();
    *slot = None;

    match Core::logout() {
        Ok(()) => serde_json::json!({ "ok": true }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_is_logged_in() -> String {
    serde_json::json!({
        "logged_in": Core::has_token_file()
    }).to_string()
}

fn build_discover_featured() -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref().unwrap().discover_featured()
    ) {
        Ok(sections) => serde_json::json!({
            "sections": sections
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_discover_albums(
    endpoint: *const c_char,
    genre_id: u64,
    offset: u32,
    limit: u32,
) -> String {
    let endpoint = match read_cstr(endpoint) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref().unwrap().discover_albums(
            &endpoint,
            genre_id,
            offset,
            limit,
        )
    ) {
        Ok(page) => serde_json::to_string(&page)
            .unwrap_or_else(|error| {
                err_json(&error.to_string())
            }),
        Err(error) => err_json(&error),
    }
}

fn build_discover_genres() -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref().unwrap().discover_genres()
    ) {
        Ok(genres) => serde_json::json!({
            "genres": genres
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_create_playlist(
    name: *const c_char,
    initial_track_id: u64,
) -> String {
    let name = match read_cstr(name) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let initial_track = if initial_track_id == 0 {
        None
    } else {
        Some(initial_track_id)
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .create_playlist(&name, initial_track)
    ) {
        Ok(playlist) => serde_json::json!({
            "ok": true,
            "playlist": playlist
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_add_track_to_playlist(
    playlist_id: u64,
    track_id: u64,
) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .add_track_to_playlist(playlist_id, track_id)
    ) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "playlist_id": playlist_id,
            "track_id": track_id
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_create_playlist_visible(
    name: *const c_char,
    initial_track_id: u64,
    is_public: bool,
) -> String {
    let name = match read_cstr(name) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let initial_track = if initial_track_id == 0 {
        None
    } else {
        Some(initial_track_id)
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .create_playlist_visible(
                &name,
                initial_track,
                is_public,
            )
    ) {
        Ok(playlist) => serde_json::json!({
            "ok": true,
            "playlist": playlist
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_remove_playlist_track(
    playlist_id: u64,
    playlist_track_id: u64,
) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .remove_playlist_track(
                playlist_id,
                playlist_track_id,
            )
    ) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "playlist_id": playlist_id,
            "playlist_track_id": playlist_track_id
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_set_playlist_public(
    playlist_id: u64,
    is_public: bool,
) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .set_playlist_public(playlist_id, is_public)
    ) {
        Ok(playlist) => serde_json::json!({
            "ok": true,
            "playlist": playlist
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_save_playlist_order(
    playlist_id: u64,
    ordered_track_ids: *const c_char,
) -> String {
    let csv = match read_cstr(ordered_track_ids) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut ids = Vec::new();

    for part in csv.split(',') {
        let trimmed = part.trim();

        if trimmed.is_empty() {
            continue;
        }

        match trimmed.parse() {
            Ok(id) => ids.push(id),
            Err(_) => return err_json(
                "invalid track id in order"
            ),
        }
    }

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .save_playlist_order(playlist_id, &ids)
    ) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "playlist_id": playlist_id
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_item_favourite_state(
    item_type: *const c_char,
    item_id: *const c_char,
) -> String {
    let item_type = match read_cstr(item_type) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let item_id = match read_cstr(item_id) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .item_favourite_state(&item_type, &item_id)
    ) {
        Ok(favourite) => serde_json::json!({
            "ok": true,
            "item_type": item_type,
            "item_id": item_id,
            "favourite": favourite
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_set_item_favourite(
    item_type: *const c_char,
    item_id: *const c_char,
    favourite: bool,
) -> String {
    let item_type = match read_cstr(item_type) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let item_id = match read_cstr(item_id) {
        Ok(value) => value,
        Err(error) => return err_json(&error),
    };

    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .set_item_favourite(
                &item_type,
                &item_id,
                favourite,
            )
    ) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "item_type": item_type,
            "item_id": item_id,
            "favourite": favourite
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_track_favourite_state(track_id: u64) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .track_favourite_state(track_id)
    ) {
        Ok(favourite) => serde_json::json!({
            "ok": true,
            "track_id": track_id,
            "favourite": favourite
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

fn build_set_track_favourite(
    track_id: u64,
    favourite: bool,
) -> String {
    let mut slot = CORE.lock().unwrap();

    if let Err(error) = ensure_core(&mut slot) {
        return err_json(&error);
    }

    match RT.block_on(
        slot.as_ref()
            .unwrap()
            .set_track_favourite(track_id, favourite)
    ) {
        Ok(()) => serde_json::json!({
            "ok": true,
            "track_id": track_id,
            "favourite": favourite
        }).to_string(),
        Err(error) => err_json(&error),
    }
}

// ---- JSON envelopes ----

fn tracks_json(tracks: &[FpTrack]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> { tracks: &'a [FpTrack] }
    serde_json::to_string(&Out { tracks }).unwrap_or_else(|e| err_json(&e.to_string()))
}

fn albums_json(albums: &[FpAlbum]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> { albums: &'a [FpAlbum] }
    serde_json::to_string(&Out { albums }).unwrap_or_else(|e| err_json(&e.to_string()))
}

fn artists_json(artists: &[FpArtist]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> { artists: &'a [FpArtist] }
    serde_json::to_string(&Out { artists }).unwrap_or_else(|e| err_json(&e.to_string()))
}

fn album_json(album: &FpAlbum, tracks: &[FpTrack]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> { album: &'a FpAlbum, tracks: &'a [FpTrack] }
    serde_json::to_string(&Out { album, tracks }).unwrap_or_else(|e| err_json(&e.to_string()))
}

fn artist_json(artist: &FpArtist, albums: &[FpAlbum]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> { artist: &'a FpArtist, albums: &'a [FpAlbum] }
    serde_json::to_string(&Out { artist, albums }).unwrap_or_else(|e| err_json(&e.to_string()))
}


fn playlists_json(playlists: &[FpPlaylist]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> {
        playlists: &'a [FpPlaylist],
    }

    serde_json::to_string(&Out { playlists })
        .unwrap_or_else(|e| err_json(&e.to_string()))
}

fn playlist_json(playlist: &FpPlaylist, tracks: &[FpTrack]) -> String {
    #[derive(serde::Serialize)]
    struct Out<'a> {
        playlist: &'a FpPlaylist,
        tracks: &'a [FpTrack],
    }

    serde_json::to_string(&Out { playlist, tracks })
        .unwrap_or_else(|e| err_json(&e.to_string()))
}

fn favourites_json(payload: &FavouritesPayload) -> String {
    serde_json::to_string(payload)
        .unwrap_or_else(|e| err_json(&e.to_string()))
}

fn err_json(msg: &str) -> String {
    format!(r#"{{"error":{}}}"#, serde_json::to_string(msg).unwrap_or_else(|_| "\"error\"".into()))
}

fn to_c(s: String) -> *mut c_char {
    CString::new(s)
        .unwrap_or_else(|_| CString::new(r#"{"error":"nul in output"}"#).unwrap())
        .into_raw()
}

#[no_mangle]
pub extern "C" fn fp_free(s: *mut c_char) {
    if s.is_null() { return; }
    unsafe { drop(CString::from_raw(s)); }
}
